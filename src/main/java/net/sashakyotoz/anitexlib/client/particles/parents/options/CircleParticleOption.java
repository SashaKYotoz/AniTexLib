...
public record CircleParticleOption(float criticalAngle, float scale, float redColor, float greenColor, float blueColor) implements ParticleOptions {
    ...
    public String writeToString() {
        return String.format(Locale.ROOT, "%s %.2f %.2f %.2f %.2f %.2f", ForgeRegistries.PARTICLE_TYPES.getKey(this.getType()), criticalAngle,scale, redColor, greenColor, blueColor);
    }

    public ParticleType<CircleParticleOption> getType() {
        return ModParticleTypes.CIRCLE_LIKE_PARTICLE.get();
    }
}